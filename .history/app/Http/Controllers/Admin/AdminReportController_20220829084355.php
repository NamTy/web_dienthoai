...
class AdminReportController extends Controller
{
    ...
    public function getReportProduct(ReportDateRequest $request)
    {
        ...
        $DateTimeAfterFormat = Carbon::parse($this->DateTimeAfter)->format('Y-m-d H:h:s');;
        // DB::enableQueryLog();
        $products = OrderDetail::whereBetween('updated_at', [$DateTimeBeforeFormat, $DateTimeAfterFormat])->get();
        // dd(DB::getQueryLog());
    ...
    {
        // DB::enableQueryLog();
        $products = OrderDetail::whereBetween('updated_at', [$DateTimeBefore, $DateTimeAfter])->get();
        // dd($products);
        // dd(DB::getQueryLog());
        PDF::setOption(['dpi' => 150, 'defaultFont' => 'sans-serif']);
    ...
    {
        $products = OrderDetail::whereBetween('updated_at', [$DateTimeBefore, $DateTimeAfter])
            ->select('product_id', 'product_name', 'product_discount', OrderDetail::raw('SUM(product_qty) as qty'))
            ->groupBy('product_id', 'product_name', 'product_discount')
            ->orderBy('qty', 'desc')
            ->get();
        PDF::setOption(['dpi' => 150, 'defaultFont' => 'sans-serif']);
        $pdf = PDF::loadView('admin.print.report-product-qty', compact('products', 'DateTimeBefore', 'DateTimeAfter'));
        return $pdf->stream('report.pdf');
    }
}
